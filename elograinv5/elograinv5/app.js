...
function renderFeed(filterTag) {
  ...
  container.innerHTML = posts.map(function(post) {
    ...
    return '<article class="card" data-id="' + post.id + '">' +
      ...
      '<button class="action-btn heart-btn ' + (post.liked ? 'liked' : '') + '" data-post="' + post.id + '" onclick="toggleLike(' + post.id + ')" aria-label="Curtir">' +
...
function toggleLike(postId) {
  ...
  if (post) {
    ...
    if (btn) { btn.classList.toggle('liked', post.liked); btn.querySelector('.cnt').textContent = post.likes; Elo.setHeart(btn, post.liked); }
    else renderFeed();
  }
}
